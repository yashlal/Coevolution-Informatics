...
def alg(MI_list_, indices_, cols_, gammas_):
    ...
    while (len(MI_list)>0) and (gamma_ind<len(gammas_)):
        ...
        with multiprocessing.Pool() as pool:
            MI_list_async = pool.starmap_async(list_comp, myargs1).get()
            MI_list = list(filter(None, MI_list_async))
            results = pool.starmap_async(mutual_inf_MP, myargs2).get()
...
if __name__=='__main__':
    epsilon, gammas = 1.16, [0.7, 0.75, 0.8, 0.85, 0.9, 0.95]
    all_species = ['Fusobacteriota', 'Cyanobacteria', 'Bacteroidota']
